            ...
            where TSchemaType : JsonSchemaBase, new()
        {
            ...
            if (schema.Type.HasFlag(SimpleType.Object))
            {
                ...
                foreach (var property in type.GetRuntimeProperties())
                {
                    ...
                    schema.Properties.Add(property.Name, jsonProperty);
                    ...
                    if (propertyTypeDescription.IsAlwaysRequired || attributes.Any(a => a.GetType().FullName == "System.ComponentModel.DataAnnotations.RequiredAttribute"))
                        schema.RequiredProperties.Add(property.Name);

                    dynamic descriptionAttribute = attributes.FirstOrDefault(a => a.GetType().FullName == "System.ComponentModel.DescriptionAttribute");
                    if (descriptionAttribute != null)
                        jsonProperty.Description = descriptionAttribute.Description;

                    dynamic regexAttribute = attributes.FirstOrDefault(a => a.GetType().FullName == "System.ComponentModel.DataAnnotations.RegularExpressionAttribute");
                    if (regexAttribute != null)
                        jsonProperty.Pattern = regexAttribute.Pattern;

                    dynamic rangeAttribute = attributes.FirstOrDefault(a => a.GetType().FullName == "System.ComponentModel.DataAnnotations.RangeAttribute");
