...
- [ ] Add the new test class and a theory for every dialect using the existing async resolver pattern from `SchemaSerializationConverterTests`. The essential XML input/assertion is:
...
- [ ] Run the new regressions red (retain failure output showing the actual missing contract):
...
- [ ] Run the red command green and existing converter/roundtrip/reference coverage:
...
- [ ] Add a failing theory for schema types and their nullable wire names (`x-nullable` for JsonSchema/Swagger2, `nullable` for OpenApi3). Use this semantic payload, substituting the nullable name:
...
- [ ] Add readonly input theories (`readOnly`, `readonly`, `READONLY`) using string and dictionary properties and all dialects through the existing serializer entry points. Assert `schema.Properties["values"].IsReadOnly`, and that accepted spellings do not appear as stray extension data. Assert dictionary entry names `Foo` and `foo` stay distinct; case-insensitive matching is for CLR schema members, not JSON dictionary keys or vendor payloads.

- [ ] Add this TypeScript regression in `DictionaryTests`, parameterizing the property spelling; copy existing imports and AAA layout:
...
- [ ] Restore the Newtonsoft special-type branch after TypeMapper handling, alongside the current JsonNode check. Follow `ReflectionServiceBase`'s full-name approach so core has no Newtonsoft assembly reference:
...
- [ ] Run green and affected projects on the shared supported target:
...
- [ ] Record S4/S5/S6/S9 against Task 1 regressions, S8/S10 against Task 2, and G1/G2 against Task 3; report exact test commands and outcomes, not merely a green build.
- [ ] Compare repaired JSON/XML and generated declarations with pre-migration `v12`/master behavior using the same fixtures. Record the exact baseline commit used; the review checkpoint is evidence, not an immutable execution baseline. Explain any intentional semantic difference and add a test/changelog note rather than blessing a snapshot.
- [ ] Confirm no new public API signatures and no core Newtonsoft package reference; inspect downstream usages read-only and list the real discriminator-mapping/derived-type scenarios still pending NSwag integration.
- [ ] Keep NSwag companion implementation/full suite deferred to its existing STJ PR. Phase 3 completion does not satisfy the final downstream merge gate. Complete the remaining roadmap phases and final NJsonSchema build/test/package/CI gates before declaring PR #1914 ready to merge.
