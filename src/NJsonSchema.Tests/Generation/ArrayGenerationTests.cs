...
namespace NJsonSchema.Tests.Generation
{
    public class ArrayGenerationTests
    {
        ...
        {
            //// Act
            var schema = await JsonSchemaGenerator.FromTypeAsync<ClassWithJArray>(new JsonSchemaGeneratorSettings { SchemaType = SchemaType.OpenApi3 });
        ...
        {
            //// Act
            var schema = await JsonSchemaGenerator.FromTypeAsync<ListContainer>(new JsonSchemaGeneratorSettings { SchemaType = SchemaType.OpenApi3 });
            var json = schema.ToJson();
